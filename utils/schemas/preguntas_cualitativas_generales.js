...
const createPreguntasCualitativasGeneralesSchema = {
  tipo: Joi.string().valid("cualitativas_generales").default('cualitativas_generales'),
  ...
  nivel_requerido: Joi.number().min(1).max(3).default(3),
  ...
  maximo_seleccionar: Joi.number().min(1).max(3).default(1),
