...
const PreguntasService = require("../../services/preguntas_postcovid");
...
} = require("../../utils/schemas/preguntas_postcovid");
...
function preguntasPostApi(app) {
  const router = express.Router();
  app.use("/api/preguntas-postcovid", router);
  ...
  router.post("/",
    ...
    validation(createPreguntasPostCovidSchema), async function(
    req,
    res,
    next
  ) {
  ...
  router.post("/procesar/test/:_id_test",
    passport.authenticate("jwt", { session: false }),
    scopesValidationHandler(['create:preguntas']),
    validateArray(createPreguntasPostCovidSchema,'preguntas'),
    async function(req,res,next) {
        try {
  ...
  router.put(
    ...
    validation(updatePreguntasPostCovidSchema),
    async function(req, res, next) {
      const { preguntaId } = req.params;
      const { body: pregunta } = req;
...
module.exports = preguntasPostApi;
