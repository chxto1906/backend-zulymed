...
class PreguntasService {
  ...
  async createPreguntas({ preguntas }) {
    for (var i = 0, l = preguntas.length; i < l; i++) {
        let abreviatura = preguntas[i].subdimension;
        let existeDimension = await this.mongoDB.findOne("subdimensiones",{abreviatura})
        if (!existeDimension){
            let data = validationLocal(createSubdimensionesSchema,{abreviatura,label: abreviatura})
            if (!data)
                return false
            else{
                let createdDimension = await this.mongoDB.create("subdimensiones",data)
                if (!createdDimension)
                    return false
            }
        }
